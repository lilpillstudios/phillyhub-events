 ...
 * Lil Pill Studios © 2026
 *
 * Fetches fresh events.json from GitHub, maps scraper fields to app fields,
 * and falls back to bundled EVENTS array if fetch fails.
 */
...
// Map scraper field names → app field names
function mapEvent(e) {
  return {
    id: e.id,
    title: e.title,
      ...
      try {
        ...
        if (cached) {
          ...
          if (age < CACHE_TTL && parsed.events?.length > 0) {
            if (!cancelled) {
              setEvents(parsed.events.map(mapEvent));
              setSource("cache");
